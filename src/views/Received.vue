    ...
    <div class="table">
      ...
      <div class="container">
        <div :key="index" v-for="(user, index) in users" class="row">
          ...
          <div class="btns">
            <span v-if="user.status == 'accept'" class="green">Солгласился</span>
            <span v-if="user.status == 'decline'" class="red">Отказался</span>
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
      users() {
        return this.$store.state.users.filter(el => el.status == "accept" || el.status == "decline")          
      },
      dir() {
